...
# 步骤1: 从prm_less_than_10_pass_questions提取问题和ground truth
questions_and_gt = []
with open('./困难数据含答案/prm_less_than_10_pass_questions.json', 'r', encoding='utf-8') as f:
    # 读取整个JSON文件
    ...
    for data in data_list:
        ...
        ground_truth = data['answer']  # 直接使用answer字段作为ground_truth
        questions_and_gt.append((question, ground_truth))

# 步骤2: 从响应文件中提取boxed答案并与ground truth比较
correct_results = []
responses = []
incorrect_indices = []  # 用于记录错误样本的索引

# 加载所有响应
with open('./requests_format_prm800k_hard_cot.jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        try:
...
for i, (question, ground_truth) in enumerate(questions_and_gt):
    ...
    try:
        ...
        if boxed_matches:
            ...
            for boxed_answer in boxed_matches:
                # 清理boxed值
                ...
                # 使用增强的验证方法
                if verify_answer(boxed_answer, ground_truth):
                    accept = True
                    break  # 找到一个匹配的就可以停止检查
            
        # 如果boxed内容验证失败，尝试直接从最后100个字符中提取数字
        ...
        incorrect_indices.append(i)  # 异常情况也视为错误
        continue

# 步骤3: 输出正确的结果到新的jsonl文件
with open('./prm800k_hard_train_with_qwen-max.jsonl', 'w', encoding='utf-8') as f:
    for result in correct_results:
        # 直接写入匹配正确的原始数据
        f.write(json.dumps(result, ensure_ascii=False) + '\n')
# 步骤4: 保存错误样本的索引到json文件
with open('./prm800k_incorrect_indices.json', 'w', encoding='utf-8') as f:
    json.dump(incorrect_indices, f)
print(f"错误样本的序号已保存到 prm800k_incorrect_indices.json")
